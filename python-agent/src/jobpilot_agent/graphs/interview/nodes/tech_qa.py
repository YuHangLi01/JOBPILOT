...
async def tech_qa_node(state: InterviewState) -> dict[str, object]:
    """技术问答：每次执行出 1 问 + 等答 + 打分，循环由条件边驱动。"""
    return await run_interview_turn(state, stage="tech_qa", max_stage_rounds=_MAX_ROUNDS)


async def judge_continue_tech_qa(state: InterviewState) -> str:
    """条件边函数：决定是继续技术问答还是推进到 scenario。

    Returns:
        "continue"：继续当前阶段
        "move_to_scenario"：推进下一阶段
    """
    rounds = (state.get("stage_round_count") or {}).get("tech_qa", 0)

    if rounds >= _MAX_ROUNDS:
        log.info("judge_tech_qa.hard_stop", rounds=rounds)
        return "move_to_scenario"
    ...
    log.info("judge_tech_qa.llm", rounds=rounds, continue_=should_continue)
    return "continue" if should_continue else "move_to_scenario"
